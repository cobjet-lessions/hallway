...
exports.init = function()
{
  ...
  // this is a "middle" db, one where entries are only read from in parallel
  if(lconfig.partition.readthrough) {
    logger.info("creating readthrough dal");
    THRUDB = dal.create(lconfig.partition.readthrough);
  }
  if(lconfig.partition.readthrough2) {
    logger.info("creating second readthrough dal");
    THRUDB2 = dal.create(lconfig.partition.readthrough2);
  }
  if(lconfig.partition.readthrough3) {
    logger.info("creating tird readthrough dal");
    THRUDB3 = dal.create(lconfig.partition.readthrough3);
  }
...
function idParse(id)
{
  if(id === "Chunks") return {table:"Chunks", hash:"00"};
  var ret = {};
  if (id.indexOf(':') > 0) {
...
exports.readFrom = function(id, cbDone)
{
  ...
  // if we're in read-through mode, also include it
  if(idp.part && THRUDB) parts.unshift({dal:THRUDB, hash:idp.hash, table:idp.table, partition:idp.part, priority:1});
  if(idp.part && THRUDB2) parts.unshift({dal:THRUDB2, hash:idp.hash, table:idp.table, partition:idp.part, priority:2});
  if(idp.part && THRUDB3) parts.unshift({dal:THRUDB3, hash:idp.hash, table:idp.table, partition:idp.part, priority:3});
  // if there's a partition, make that one preferred
  if(idp.part) parts.unshift({dal:PARTDB, hash:idp.hash, table:idp.table, partition:idp.part, priority:4});
  cbDone(parts);
};
...
exports.writeTo = function(id, cbDone)
{
  ...
  if(PART_SIZE && !lconfig.chunks) logger.warn("fallback write to central Entries table",id);
...
exports.cleanUp = function(id, cbDone)
{
  ...
  PARTDB.query("SELECT hex(idr) FROM "+table+" WHERE idr = x? LIMIT 1", [idp.hash], function(err, rows){
    ...
    dal.query("DELETE FROM Entries WHERE idr = x? LIMIT 1", [idp.hash], function(){
      if(!THRUDB) return cbDone();
      // also try to delete from the readthrough
      THRUDB.query("DELETE FROM Entries WHERE idr = x? LIMIT 1", [idp.hash], function(){
        if(!THRUDB2) return cbDone();
        THRUDB2.query("DELETE FROM Entries WHERE idr = x? LIMIT 1", [idp.hash], cbDone);        
      });
    });
  });
};
