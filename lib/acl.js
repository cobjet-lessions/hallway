...
exports.getApp = function(app, callback, refresh) {
  ...
  redis.get(app, function (error, result) {
    var js;

    try {
      js = JSON.parse(result);
    } catch (E) {
      logger.warn("Failed to parse result from getApp.redis for " + app + ": " + E);
    }

    if (js && typeof js === 'object' && !refresh) {
    ...
    dal.query(q, [app], function (err, rows) {
      ...
      if (rows[0]) {
        ...
        redis.set(app, JSON.stringify(rows[0]), function (err) {
          if (err) logger.warn("Failed to save app in redis for " + app + ": " + err);
        });
      }
...
exports.getAppClasses = function(app, callback) {
  ...
  function doApp(err, app) {
    ...
    // make a class for each synclet in the custom frequency alone
    if (app.notes.customOnly && typeof app.notes.customFreqs !== 'object') logger.warn("misconfigured app, customOnly and no customFreqs", app.app);
    if (app.notes.customOnly && typeof app.notes.customFreqs === 'object') {
      classes = {};
      Object.keys(app.notes.customFreqs).forEach(function(service){
        Object.keys(app.notes.customFreqs[service]).forEach(function(synclet){
          classes[service+'_'+synclet] = true;
        });
      });
      return process.nextTick(callback.bind(null, null, classes));
    }
    ...
    // explicitly deny social features unless social override is set
    if (app.notes[SOCIAL_OVERRIDE]) {
      classes.social = true;
    }
    return callback(null, classes);
  };

  // pass in a known app object already, was added to make the acl tests possible
  if(typeof app === 'object') return doApp(null, app);

  // got an app id, look it up then process
  exports.getApp(app, doApp);
};
...
function appCustomFreq(app, service, synclet, callback) {
  exports.getApp(app, function(err, appInfo) {
    ...
    if (!appInfo) return callback('no app ' + app);
...
exports.customFreq = function(apps, service, synclet, callback) {
  ...
  async.forEachSeries(apps, function(app, cbLoop) {
    // only sync configured whitelisted apps
    if (lconfig.synclist && !lconfig.synclist[app]) return cbLoop();
    // don't sync any blacklisted apps here
    if (lconfig.syncskip && lconfig.syncskip[app]) return cbLoop();
