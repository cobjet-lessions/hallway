...
exports.setOneCat = function(id, cat, options, cbDone){
  ...
  logger.debug("setOneCat",id,cat,options,sql,binds);
...
exports.initDB = function(callback) {
  ...
  logger.vital('Connecting to memcache...');

  memcache.connect(function() {
    logger.vital("Connected to memcache");
...
IJOD.prototype.commitAddTransaction = function(timings, cbDone) {
  ...
  if(lconfig.chunks)
  {
    var writeBuffer = zlib.compress(new Buffer(this.transactionItems.join("\n")));
  }else{
  ...
  backend.put(self.path, writeBuffer, function (err) {
    ...
    writeBuffer = null;
    if (!err) {
      logger.debug("Saving %d entries to ijod for %s", self.transactionItems.length, self.basePath);
...
IJOD.prototype.addData = function(timings, arg) {
  ...
  if (!arg.data || !arg.types) return;

  // TODO: This doesn't call any async code and can be converted
  Object.keys(arg.types).forEach(function(type) {
...
exports.getChunks = function(basePath, range, cbEach, cbDone) {
  var at1 = parseInt(range.since || 0);
  var at2 = parseInt(range.until || Date.now());
  
  var sql = "SELECT path FROM Chunks WHERE base = unhex(?) and at2 > ? and at1 <= ? order by at2 desc";
  var binds = [
    idr.baseHash(basePath),
    at1, at2
  ];
  ...
  dal.query(sql, binds, function(error, rows) {
    ...
    if (rows.length === 0) return cbDone();

    var results = 0;

    // Process the curPath
    function processPath(row, cbPath) {
      backend.get(row.path, 0, 0, function(err, buf) {
        if(err) logger.debug("error fetching",row.path,err);
        if (err) return cbPath(null);
        var pieces = [];
        var decompressed = zlib.uncompress(buf);
        if (!decompressed) return cbPath(null, []);
        var chunks = decompressed.toString().split("\n");
        chunks.map(function(chunk){
          if(!chunk || chunk === "") return;
          var ret;
          try { ret = JSON.parse(chunk); } catch(E) { logger.warn("couldn't parse chunk",basePath,row.path,chunk); }
          return ret;
        }).forEach(function(entry){
          if(!entry) return;
          if(range.limit && results >= range.limit) return;
          if(entry.at < at1 || entry.at > at2) return;
          results++;
          cbEach(entry);
        });
        if(range.limit && results >= range.limit) return cbPath("done");
        cbPath();
      });
    }

    var startS3Time = Date.now();
    async.forEachSeries(rows, processPath, function() {
      logger.debug("Chunks count %d paths %d run time sql %d s3 %d", results, rows.length, (Date.now() - startQueryTime), (Date.now() - startS3Time));
      cbDone();
    });
  });
};
...
exports.getRange = function(basePath, range, cbEach, cbDone) {
  if(lconfig.chunks) return exports.getChunks(basePath, range, cbEach, cbDone);
...
exports.chunkAdd = function(entries, callback) {
  ...
  var baseHash = idr.baseHash(entries[0].idr);
  logger.debug("chunk add", pidPath, entries.length);
  var ij = new IJOD(pidPath);
  ij.basePath = basePath;
  var timings = {};
  var start = Date.now();
  ...
  entries.forEach(function(entry) {
    ...
    ij.addData(timings, entry);
    entry.chunk = ij.path;
    if(!at1 || entry.at < at1) at1 = entry.at;
    if(!at2 || entry.at > at2) at2 = entry.at;
  });
  at1 = parseInt(at1 / 1000);
  at2 = parseInt(at2 / 1000);
  timings.addData = Date.now() - start;
  ij.transactionQueries = [];
  ij.transactionQueries.push({id:"Chunks", sql:"insert into Chunks (base, path, at1, at2, count) values (unhex(?), ?, ?, ?, ?)", binds:[baseHash, ij.path, at1, at2, entries.length]});
  start = Date.now();
...
//  }).join(",");
// temp disable hash dup checking
...
exports.getChunksBounds = function(basePath, range, cbDone) {
  var at1 = parseInt((range.since || 0)/1000);
  var at2 = parseInt((range.until || Date.now())/1000);
  
  var sql = "SELECT min(at1) as oldest, max(at2) as newest, sum(count) as total FROM Chunks WHERE base = unhex(?) and at2 > ? and at1 <= ?";
  var binds = [
    idr.baseHash(basePath),
    at1, at2
  ];
  ...
  var startQueryTime = Date.now();
  dal.query(sql, binds, function(error, rows) {
    if (error || !rows || rows.length != 1) return cbDone(error);
    if(rows[0].total > 0) return cbDone(null, rows[0]);
    cbDone(null, {newest:0, oldest:0, total:0});
  });
};

// just quickly return the at bounds of a potential range request
exports.getBounds = function(basePath, range, cbDone) {
  if(lconfig.chunks) return exports.getChunksBounds(basePath, range, cbDone);
...
exports.pump = function(arg, cbDone) {
  ...
  async.forEachSeries(Object.keys(bases), function(base, cb) {
    var fun = lconfig.chunks ? exports.chunkAdd : exports.batchSmartAdd;
    fun(bases[base], function(error, timing) {
