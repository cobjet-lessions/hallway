...
exports.getChunks = function(basePath, range, cbEach, cbDone) {
  ...
  var sql = "SELECT path FROM Chunks WHERE base = unhex(?) and at1 > ? and at2 < ? ";
  var binds = [
  ...
  dal.query(sql, binds, function(error, rows) {
    ...
    function processPath(row, cbPath) {
      backend.get(row.path, 0, 0, function(err, buf) {
        ...
        cbPath(null, chunks.map(function(chunk){ return JSON.parse(chunk); }));
      });
    }

    var startS3Time = Date.now();
    async.map(rows, processPath, function(error, results) {
      for (var i = 0; i < results.length; ++i) {
        if (!results[i]) continue;
        entries.filter(results[i], range).forEach(cbEach);
      }
      logger.debug("Chunk run time sql %d s3 %d", (Date.now() - startQueryTime), (Date.now() - startS3Time));
