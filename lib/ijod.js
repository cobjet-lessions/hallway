...
exports.getChunks = function(basePath, range, cbEach, cbDone) {
  ...
  var sort = (range.sort == "asc") ? "asc" : "desc";
  
  var sql = "SELECT path FROM Chunks WHERE base = unhex(?) and at2 > ? and at1 <= ? order by at2 "+sort;
  var binds = [
    ...
    function processPath(row, cbPath) {
      backend.get(row.path, 0, 0, function(err, buf) {
        ...
        chunks.map(function(chunk){
          ...
          if((entry.at/1000) < at1 || (entry.at/1000) > at2) return;
          if(ndx[entry.id]) return;
          results++;
          ndx[entry.id] = true;
          cbEach(entry);
        });
