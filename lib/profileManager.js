...
function genGetLocal(profile, cbDone) {
  ...
  KVSTORE.get('profiles', profile.id, {}, function(err, data) {
    ...
    if(data) return cbDone(err, _.extend(profile, data));
    logger.warn("genGetLocal failed, falling up",profile.id,err);
    genGetRemote(profile, cbDone);
  });
}
...
function allSetLocal(id, newAuth, newConfig, cbDone) {
  ...
  if (newAuth || newConfig) {
    genGet(id, function (err, profile) {
      ...
      if (typeof profile.auth != "object") profile.auth = {};
      if (typeof profile.config != "object") profile.config = {};
      ...
      if (newAuth) {
        ...
        if (typeof profile.auth.apps != "object") profile.auth.apps = {};
