...
exports.authSet = function(id, app, newAuth, cbDone) {
  var buf = new Buffer(JSON.stringify(newAuth));
  backend.put('auths/'+app+'/'+newAuth.account+'_'+id, buf, function (err) {
    if(err) logger.error("AUTH BACKUP FAILED", id, app, newAuth);
    cbDone(err);
  });
};
