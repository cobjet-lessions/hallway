...
function genGetLocal(profile, cbDone) {
  ...
  KVSTORE.get('profiles', profile.id, {}, function(err, data) {
    logger.debug("profile fetch time", Date.now() - start, data && data.config && data.config.set);
    // reset the config based on the config file
    if(data && lconfig.chunks && lconfig.chunks.configReset)
    {
      if(!data.config) data.config = {};
      if(!data.config.set || data.config.set < lconfig.chunks.configReset) data.config = {set:lconfig.chunks.configReset};
    }
...
exports.reset = function(id, cbDone) {
  exports.loadProfile(id, function(err, profile) {
    ...
    else return exports.resetLocal(id, cbDone);
  });
};
...
exports.resetLocal = function(id, cbDone) {
  genGet(id, function(err, profile) {
    if (err) return cbDone(err);
    profile.config = {};
    KVSTORE.put('profiles', id, profile, cbDone);
