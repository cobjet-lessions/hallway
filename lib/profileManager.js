...
function genGet(id, cbDone, app) {
  ...
  exports.loadProfile(id, function(err, profile) {
    ...
    if (app && lconfig.pods.apps && lconfig.pods.apps[app])
    {
      profile.pod = lconfig.pods.apps[app];
      logger.debug("setting pod for app", profile.pod, id, app);
    }
...
exports.authGetAcct = function(id, app, acct, cbDone) {
  if(!backend) return exports.authGet(id, app, cbDone);
  backend.get('auths/'+app+'/'+acct+'_'+id, 0, 0, function (err, buf) {
    ...
    exports.authGet(id, app, function(err, auth){
      cbDone(err, auth);
      // save to s3
      if(backend && auth) backend.put('auths/'+app+'/'+acct+'_'+id, new Buffer(JSON.stringify(auth)), function (err) {
        if(err) logger.warn("AUTH BACKUP FAILED", id, app, auth);
        logger.info("authGetAcct migration",'auths/'+app+'/'+acct+'_'+id);
      });
    });
  });
}
...
exports.authGet = function(id, app, cbDone) {
  ...
  genGet(id, function(err, obj){
    ...
    logger.info("authGet old",app,id);
    cbDone(null, auth);
  }, app);
};
...
exports.authSet = function(id, app, newAuth, cbDone) {
  if(backend) backend.put('auths/'+app+'/'+newAuth.account+'_'+id, new Buffer(JSON.stringify(newAuth)), function (err) {
  ...
  exports.allSet(id, newAuth, null, cbDone, app);
};

exports.allSet = function(id, newAuth, newConfig, cbDone, appID) {
  logger.debug('Setting profile data for', id);
  exports.loadProfile(id, function(err, profile) {
    if (err) return cbDone(err);
    if (!profile) return cbDone(new Error('No profile found: ' + id));
    var pod = (lconfig.pods.apps && lconfig.pods.apps[appID]) || profile.pod;

    if (pod) allSetRemote(pod, id, newAuth, newConfig, cbDone);
  ...
  async.forEach(bases, function(base, cbBases){
    ...
    exports.authGetAcct(idr.pid(base), options.app, options.account, function (err, auth) {
      ...
      self.sync({auth: auth, config: {}}, function (err, data) {
        ...
        // If we're refreshing save it back out
        if (auth.token.refresh_token && data.auth.token.refresh_token && auth.token.refresh_token != data.auth.token.refresh_token) {
          exports.authSet(id, options.app, data.auth, cbBases);
        } else {
          cbBases();
        }
      });
