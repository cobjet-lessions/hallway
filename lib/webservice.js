  ...
  entries.runBases(bases, {limit: 1}, function (item) {
    ...
    if (!self) {
      profileManager.getCached(pid, function(){}, function(err, profiles) {
        if (err || !profiles) {
          logger.warn('No /profile/* for pid', pid);
          return res.jsonErr('No data', 404);
        }
...
hallway.get('/profiles/:serviceName/:serviceEndpoint/synced', function (req, res) {
  ...
  if (lconfig.pods.apps && lconfig.pods.apps[req._authsome.app])
  {
    podClient.getSynced(lconfig.pods.apps[req._authsome.app], pid, req.params.serviceEndpoint, function(err, synced){
      if(err) return res.jsonErr(err);
      res.json(synced, synced?200:204);
    });
    return;
  }
...
hallway.get('/friends/:group', function (req, res) {
  ...
  friends.baseMerge(bases, options, function (err, list) {
    ...
    async.forEach(list.slice(options.offset, options.offset + options.limit),
      ...
      if(friend.peer) fprof.peer = friend.peer;
      async.forEach(friend.profiles, function (profile, cbProfs) {
        if (profile.base === options.ace) return process.nextTick(cbProfs);
  ...
  async.forEachSeries(profiles, function (item, cbProfile) {
    profileManager.reset(item.profile, function (err) {
      ...
      async.forEach(dMap.bases([item.profile]), ijod.spoil, function () {
        taskmanNG.syncNow(item.profile, 1, cbProfile);
      });
    });
  }, function (err) {
...
hallway.get('/id/:id', function (req, res) {
  ...
  podClient.getOne(id, profiles(req._authsome.profiles), function (err, entry) {
    if (err) logger.warn("/id/",id,err);
    if (err) return res.jsonErr(err, 404);
    if (!entry) return res.jsonErr('Not Found', 404);
    var pid = idr.pid(entry.idr);
    if (pid.indexOf('@') > 0 &&
    ...
    if (mediaf) {
      var app = req._authsome.app;
      return profileManager.authGetAcct(pid, app, req._authsome.account, function (err, auth) {
  ...
  }, req._authsome.app, req._authsome.account);
});
